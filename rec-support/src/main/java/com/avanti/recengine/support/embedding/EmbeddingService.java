package com.avanti.recengine.support.embedding;
 ...
 * HuggingFace tokenizer — no Python inference server, no hosted embedding
 * API. Ported from the sibling {@code search} project's
 * {@code EmbeddingService}: this is the one piece of that project reused
 * directly, since it's self-contained (no Spring, no Elasticsearch) and
 * Pinecone Local (unlike hosted Pinecone) has no integrated inference, so
 * embeddings must be generated client-side regardless.
 *
 * <p>BGE models are trained with CLS-token pooling (the first token of
 * last_hidden_state), not mean pooling, and require an asymmetric instruction
 * prefix on queries only — both easy to get wrong silently since a
 * mean-pooled embedding still "works," just worse.
 */
